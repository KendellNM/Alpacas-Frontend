...
interface AlpacaApiService {
    ...
    @GET("alpacas")
    suspend fun getAlpacasByGanadero(
        @Query("ganadero_id") ganaderoId: Long
    ): Response<List<AlpacaDto>>
    
    @GET("alpacas/{id}")
    suspend fun getAlpacaById(
        @Path("id") id: Long
    ): Response<AlpacaDto>
    
    @POST("alpacas")
    suspend fun createAlpaca(
        @Body request: AlpacaCreateRequest
    ): Response<AlpacaDto>
    
    @PUT("alpacas/{id}")
    suspend fun updateAlpaca(
        @Path("id") id: Long,
        @Body request: AlpacaCreateRequest
    ): Response<AlpacaDto>
    
    @DELETE("alpacas/{id}")
    suspend fun deleteAlpaca(
        @Path("id") id: Long
    ): Response<Unit>
}
